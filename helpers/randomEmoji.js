...
import { fetchEmojis } from 'emojibase'; // Import the fetchEmojis function

/**
 * Outputs a random emoji object of a given category
 * @param {String} category="Animals" - Starting string of category name
 * @returns {Object} Random emoji object
 */
const randomEmoji = async (category = 'Animals') => {
  const emojis = await fetchEmojis(); // Fetch all emojis
  const animalEmojis = emojis.filter((emoji) => {
    return emoji.category === category && emoji.version <= 8;
  });
